...
<template>
  <tr>
    ...
    <td>
      <input type="text" placeholder="Add a description..." v-model="position().title"
    ...
    <td>
      <div class="form-control">
        <div class="input-group">
          <input type="text" placeholder="0" v-model="position().value" v-maska="{
            mask: '#*.##',
            preprocessor: (val) => {
          ...
          <select class="select select-bordered select-sm bg-base-300" v-model="position().valueType">
            ...
            <option value="fixed">
              {{useCountries.currencies.find((c) => c.cc ===
                useProfile().me.organization.settings.general.currency).symbol }}
            </option>
          </select>
        </div>
      </div>
